...
class RateLimitWindow:
    """速率限制滑動窗口"""
    rpm: int  # 每分鐘請求數
    itpm: int  # 每分鐘輸入 tokens
    otpm: int  # 每分鐘輸出 tokens
    ...
    def get_wait_time(self, input_tokens_needed: int, output_tokens_needed: int) -> float:
        """計算需要等待的時間（秒）"""
        with self.lock:
            self._clean_window()
            ...
            if current_input_tokens + input_tokens_needed > self.itpm:
                # 找出需要等待多久才能有足夠的配額
                tokens_to_free = (current_input_tokens + input_tokens_needed) - self.itpm
                accumulated = 0
                for timestamp, tokens in self.input_tokens:
                    accumulated += tokens
            ...
            if current_output_tokens + output_tokens_needed > self.otpm:
                # 找出需要等待多久才能有足夠的配額
                tokens_to_free = (current_output_tokens + output_tokens_needed) - self.otpm
                accumulated = 0
                for timestamp, tokens in self.output_tokens:
                    accumulated += tokens
                    if accumulated >= tokens_to_free:
                        wait_time = 60 - (current_time - timestamp)
                        if wait_time > 0:
    ...
    def record_request(self, input_tokens_used: int, output_tokens_used: int):
        """記錄一次請求的 token 使用"""
        with self.lock:
            current_time = time.time()
            self.request_times.append(current_time)
            self.input_tokens.append((current_time, input_tokens_used))
            self.output_tokens.append((current_time, output_tokens_used))
            self._clean_window()
    ...
    def get_current_usage(self) -> Dict[str, int]:
        ...
        with self.lock:
            ...
            return {
                ...
                'input_tokens': sum(tokens for _, tokens in self.input_tokens),
                'output_tokens': sum(tokens for _, tokens in self.output_tokens),
                'rpm_limit': self.rpm,
                'itpm_limit': self.itpm,
                'otpm_limit': self.otpm
            }
...
class MultiModelRateLimiter:
    ...
    def get_limiter(self, model: str) -> RateLimitWindow:
        """獲取特定模型的速率限制器"""
        with self.lock:
            if model not in self.limiters:
                model_config = MODEL_LIMITS.get(model)
                if model_config and 'rate_limits' in model_config:
                    limits = model_config['rate_limits']
                    self.limiters[model] = RateLimitWindow(
                        rpm=limits['rpm'],
                        itpm=limits['itpm'],
                        otpm=limits['otpm']
                    )
    ...
    def check_and_wait(self, model: str, input_tokens: int, output_tokens: int) -> float:
        """檢查並返回需要等待的時間"""
        limiter = self.get_limiter(model)
        return limiter.get_wait_time(input_tokens, output_tokens)
