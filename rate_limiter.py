...
class TokenRateLimiter:
    ...
    def can_use_tokens(self, tokens_needed):
        """檢查是否可以使用指定數量的 tokens"""
        with self.lock:
            now = datetime.now()
            one_minute_ago = now - timedelta(minutes=1)
            
            # 清理舊記錄
            self.token_usage = [(ts, tk) for ts, tk in self.token_usage if ts > one_minute_ago]
            
            # 計算當前使用量
            current_usage = sum(tk for ts, tk in self.token_usage)
            
            return current_usage + tokens_needed <= self.tokens_per_minute
    ...
    def get_wait_time(self, tokens_needed):
        ...
        with self.lock:
            ...
            if current_usage + tokens_needed <= self.tokens_per_minute:
                return 0
            
            # 找出最早的記錄
            if self.token_usage:
                oldest_timestamp = min(ts for ts, tk in self.token_usage)
                wait_time = (oldest_timestamp + timedelta(minutes=1) - now).total_seconds()
                return max(0, wait_time + 1)  # 額外加 1 秒緩衝
